...
# copy the files
images = np.sort(glob.glob(img_dl_path + "/*.png"))
label_vectors = np.sort((glob.glob(labels_dl_path + "/*.txt")))

for idx, (img, lbl) in enumerate(zip(images, label_vectors)):
    print(idx, img, lbl)
    if idx % 2 == 0:
...
# create the dataset file if it does not exist already
